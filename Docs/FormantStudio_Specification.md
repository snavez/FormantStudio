...
The CSV is anchored on an explicitly chosen **primary tier** (a dropdown on the tier page): exactly one row per labelled segment when it is an interval tier, or one row per point when it is a point tier. Tiers are otherwise include/exclude only — there is no manual reordering, and columns appear in TextGrid file order. All other columns are matched to the row token purely by time, in either direction: another interval tier contributes the label of the segment encapsulating the token, or the `; `-joined labels of the segments subdividing it (e.g. an `allophone` column reading `t0; tH` under a `/t/` row); point tiers contribute the marks falling inside the token. A point primary contributes a single `<tier>_time` column (a point has no width). There are no unconditional bounds columns: duration extraction is configured per interval tier with two checkboxes — *Duration* emits `<tier>_dur`, and *start/end times* (available once Duration is ticked) additionally emits `<tier>_start`/`<tier>_end` for the interval the row resolved to on that tier. The redundancy between `<tier>_dur` and `end − start` is deliberate: duration is the graphable value, the bounds are traceability anchors, and each is opt-in.

All measurement columns are wide and one-row-per-token. At-point formants use numbered sets sized by the corpus-wide maximum of points per row token, filled in time order: `<tier>1_time`, `F1_<tier>1`, `F2_<tier>1`, `F3_<tier>1`, then `…<tier>2…`, with unused sets left blank (a monophthong with one target leaves the `Target2` set empty). Segment formants are `F1_<pct>%`/`F1_<ms>ms` per marker. Categorisation columns are appended when enabled.

Spectral columns are seven per percentage marker: `COG_<pct>%`, `SD_<pct>%`, `skew_<pct>%`, `kurt_<pct>%`, `winms_<pct>%` (the effective window in ms), `winsource_<pct>%`, and `nsamples_<pct>%`. Each analysis window is **symmetric about its marker and never crosses a segment edge**: the centre is not moved to make a window fit and the window is never one-sidedly truncated. By default the width is **proportional** — `w_prop` (30%) of the segment duration, capped at 30 ms — so window artefacts do not correlate with segment duration; a fixed-ms mode is also available. When a proportional/fixed window will not fit symmetrically it falls back to the minimum window (5 ms), and if even that will not fit the marker is unmeasurable: its moments are blank and `winsource` is `too_short`. `winsource` is one of `proportional | clamped_max | fixed | clamped_min | too_short`, and `nsamples` is the window's signal-sample count, both for post-hoc reliability filtering.
